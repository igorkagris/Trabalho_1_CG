...
def fill_poly():                #algoritmo de coloração fill_poly
    ...
    para cada valor (linha de pixel) entre o menor valor em y e o maior
        ...
        corA_B = cor atual do degrade entre A e B, ou B e C
        corA_C = cor atual do degrade entre os vertices A e C
        ...
        para cada elelento entre x1 e x2
            ...
            cor_pixel = cor atual do degrade entre corA_B e corA_C, convertido para hexadecimal novamente
    ...
    corA_B = (int((1-degrade)*cor_A_rgb[0] + degrade*cor_B_rgb[0]), int((1-degrade)*cor_A_rgb[1] + degrade*cor_B_rgb[1]), int((1-degrade)*cor_A_rgb[2] + degrade*cor_B_rgb[2]))

    tx_cor= (1 / (obj.B.y - obj.A.y)) * cor_B_rgb
    corA_B = (cor_A_rgb + tx_cor)

    '''
    txA_B = (obj.B.x - obj.A.x) / (obj.B.y - obj.A.y)
    txB_C = (obj.C.x - obj.B.x) / (obj.C.y - obj.B.y)
    txA_C = (obj.C.x - obj.A.x) / (obj.C.y - obj.A.y)
    x1 = obj.A.x
    x2 = obj.A.x

    tx_corA_B = tx_corA_C = tx_corB_C = [0.01, 0.01, 0.01] #declaração
    
    #tx_corA_B[0] = propagacao_cor * (cor_B_rgb[0] - cor_A_rgb[0])
    #tx_corA_B[1] = propagacao_cor * (cor_B_rgb[1] - cor_A_rgb[1])
    #tx_corA_B[2] = propagacao_cor * (cor_B_rgb[2] - cor_A_rgb[2])
    #propagacao_cor = (1 / (obj.B.y - obj.A.y))
    tx_corA_B = [((1 / (obj.B.y - obj.A.y)) * (cor_B_rgb[x] - cor_A_rgb[x])) for x in range(len(tx_corA_B))]

    tx_corA_C = [((1 / (obj.C.y - obj.A.y)) * (cor_C_rgb[x] - cor_A_rgb[x])) for x in range(len(tx_corA_C))]

    tx_corB_C = [((1 / (obj.C.y - obj.B.y)) * (cor_C_rgb[x] - cor_B_rgb[x])) for x in range(len(tx_corB_C))]
    corA_B = corA_C = cor_A_rgb

    for y in range((obj.A.y+1), (obj.C.y - 1)):

        if (y < obj.B.y):
            x1 += txA_B
            corA_B = [x + y for x, y in zip(corA_B, tx_corA_B)] #corA_B += tx_corA_B (para os valores, sem concatenar os vetores)
        ...
        x2 += txA_C
        corA_C = [x + y for x, y in zip(corA_C, tx_corA_C)]
        ...
        if (x1<x2):
            ...
            cor_pixel = corA_C
            tx_cor_pixel = [(tx_cor_pixel[x] * (-1)) for x in range(len(tx_cor_pixel))]

        for i in range(round(min(x1, x2)), (round(max(x1, x2)))-1 ):
            cor_pixel = [x + y for x, y in zip(cor_pixel, tx_cor_pixel)]
            cor_pixel_int = [int(round(x)) for x in cor_pixel]
            cor_hex = ("#{:02X}{:02X}{:02X}".format(*cor_pixel_int))
            janela_desenho.create_oval(i, y, i+1, y+1, fill= cor_hex, outline=cor_hex)
